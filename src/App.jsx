...
function Setup({game,setGame,onSave,onLaunch,onBack,onLogout,busy}) {
  ...
  return <><Header title={game.title} subtitle="آماده‌سازی بازی" onBack={onBack} onLogout={onLogout}/><main className="container page">
    ...
    <div className="circles">{game.circles.map((circle,ci)=><section className="circle-panel" key={circle.id} style={{'--circle':circle.color}}><div className="circle-title"><input type="color" value={circle.color} onChange={e=>updateCircle(ci,'color',e.target.value)}/><input value={circle.name} onChange={e=>updateCircle(ci,'name',e.target.value)}/></div>
      <div className="wizard-grid">{circle.wizards.map((wizard,wi)=><article className="wizard-setup" key={wizard.id}><span className="wizard-number">{fa.format(wi+1)}</span><Field label="چهره"><input value={wizard.face} onChange={e=>updateWizard(ci,wi,'face',e.target.value)}/></Field><Field label="نام حقیقی"><input value={wizard.trueName} onChange={e=>updateWizard(ci,wi,'trueName',e.target.value)}/></Field><Field label="دو طلسم سطح یک"><select multiple value={wizard.knownSpells} onChange={e=>updateWizard(ci,wi,'knownSpells',[...e.target.selectedOptions].map(o=>o.value))}>{LEVEL_ONE_SPELLS.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></Field></article>)}</div>
    </section>)}</div>
  </main></>
}
...
function Overview({game,onNight,onHistory,onUndo,onArchive,onDelete,onBack,onLogout}) {
  ...
  return <><Header title={game.title} subtitle={game.status==='completed'?'بازی پایان یافته':`پیش از شب ${fa.format(game.nightNumber)}`} onBack={onBack} onLogout={onLogout} actions={<Button variant="small" onClick={onHistory}>تاریخچه</Button>}/><main className="container page">
    ...
    <div className="circle-overview">{game.circles.map(c=><section key={c.id} className="overview-card" style={{'--circle':c.color}}><div><i></i><h3>{c.name}</h3>{c.knowledgeCooldown>0&&<span className="cooldown">دانش: {fa.format(c.knowledgeCooldown)} شب</span>}</div>{c.wizards.map(w=><div className={`wizard-row ${!w.alive?'dead':''}`} key={w.id}><span>{w.face}<small>{w.trueName}</small></span><span>سطح {fa.format(w.level)}<small>{fa.format(w.links.length)} پیوند · {fa.format(w.knownSpells.length)} ورد</small></span></div>)}</section>)}</div>
...
function randomOptions(game,action,spell,wizard,living) {
  ...
  if(spell?.random==='unknownAtLevel')return SPELLS.filter(s=>s.level===wizard.level&&!wizard.knownSpells.includes(s.id)).map(s=>({id:s.id,label:s.name}))
  if(spell?.random==='unknownLevelOne')return SPELLS.filter(s=>s.level===1&&!wizard.knownSpells.includes(s.id)).map(s=>({id:s.id,label:s.name}))
  if(spell?.random==='targetKnownSpell')return target?target.knownSpells.filter(id=>!wizard.knownSpells.includes(id)).map(id=>({id,label:SPELL_BY_ID[id].name})):[]
...
function ActionCard({game,wizard,action,update,warnings,living}) {
  ...
  return <article className={`action-card ${warnings.length?'has-warning':''}`}><div className="action-title"><div><h4>{wizard.face}</h4><span>سطح {fa.format(wizard.level)} · {wizard.trueName}</span></div><label className="force"><input type="checkbox" checked={action.force} onChange={e=>update(wizard.id,'force',e.target.checked)}/> اجرای اجباری</label></div>
    <div className="action-fields"><Field label="طلسم"><select value={action.spellId} onChange={e=>update(wizard.id,'spellId',e.target.value)}><option value="">انتخاب کنید…</option>{SPELLS.filter(s=>s.level===0||wizard.knownSpells.includes(s.id)).map(s=><option key={s.id} value={s.id}>{s.name} · سطح {fa.format(s.level)}</option>)}</select></Field>
