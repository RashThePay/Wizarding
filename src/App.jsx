...
import { LEVEL_ONE_SPELLS, SPELL_BY_ID, SPELLS, TARGET, targetLabel, UNKNOWN_NAME } from './domain/rules.js'
...
function Setup({game,setGame,onSave,onLaunch,onBack,onLogout,busy}) {
  ...
  return <><Header title={game.title} subtitle="آماده‌سازی بازی" onBack={onBack} onLogout={onLogout}/><main className="container page">
    ...
    <div className="circles">{game.circles.map((circle,ci)=><section className="circle-panel" key={circle.id} style={{'--circle':circle.color}}><div className="circle-title"><input type="color" value={circle.color} onChange={e=>updateCircle(ci,'color',e.target.value)}/><input value={circle.name} onChange={e=>updateCircle(ci,'name',e.target.value)}/></div>
      <div className="wizard-grid">{circle.wizards.map((wizard,wi)=><article className="wizard-setup" key={wizard.id}><span className="wizard-number">{fa.format(wi+1)}</span><div className="wizard-identity"><Avatar index={wizard.avatarIndex}/><div><Field label="چهره"><input value={wizard.face} onChange={e=>updateWizard(ci,wi,'face',e.target.value)}/></Field><Field label="نام حقیقی"><input value={wizard.trueName} onChange={e=>updateWizard(ci,wi,'trueName',e.target.value)}/></Field></div></div><Field label="انتخاب پرتره"><AvatarPicker value={wizard.avatarIndex} onChange={value=>updateWizard(ci,wi,'avatarIndex',value)}/></Field><Field label="دو طلسم اولیه"><select multiple value={wizard.initialSpells} onChange={e=>updateWizard(ci,wi,'initialSpells',[...e.target.selectedOptions].map(o=>o.value))}>{LEVEL_ONE_SPELLS.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></Field></article>)}</div>
    </section>)}</div>
  </main></>
}
...
function Overview({game,onNight,onHistory,onUndo,onArchive,onDelete,onBack,onLogout}) {
  ...
  return <><Header title={game.title} subtitle={game.status==='completed'?'بازی پایان یافته':`پیش از شب ${fa.format(game.nightNumber)}`} onBack={onBack} onLogout={onLogout} actions={<Button variant="small" onClick={onHistory}>تاریخچه</Button>}/><main className="container page">
    ...
    <div className="circle-overview">{game.circles.map(c=><section key={c.id} className="overview-card" style={{'--circle':c.color}}><div><i></i><h3>{c.name}</h3>{c.knowledgeCooldown>0&&<span className="cooldown">دانش: {fa.format(c.knowledgeCooldown)} شب</span>}</div>{c.wizards.map(w=><div className={`wizard-row ${!w.alive?'dead':''}`} key={w.id}><div className="wizard-face"><Avatar index={w.avatarIndex} size="xs"/><span>{w.face}<small>{w.trueName}</small></span></div><span>سطح {fa.format(w.level)}<small>{fa.format(w.links.length)} پیوند</small></span></div>)}</section>)}</div>
...
function ActionCard({game,wizard,action,update,warnings,living}) {
  ...
  return <article className={`action-card ${warnings.length?'has-warning':''}`}><div className="action-title"><div className="wizard-face"><Avatar index={wizard.avatarIndex} size="sm"/><div><h4>{wizard.face}</h4><span>سطح {fa.format(wizard.level)} · {wizard.trueName}</span></div></div><label className="force"><input type="checkbox" checked={action.force} onChange={e=>update(wizard.id,'force',e.target.checked)}/> اجرای اجباری</label></div>
    ...
    {spell?.target===TARGET.FACE&&<WizardSelect label="چهرهٔ هدف" value={action.targetWizardId} onChange={v=>update(wizard.id,'targetWizardId',v)} living={living}/>} {spell?.target===TARGET.NAME&&<TrueNameSelect value={action.spokenName} onChange={v=>update(wizard.id,'spokenName',v)} living={living}/>} 
...
function TrueNameSelect({value,onChange,living}) { return <Field label="نام حقیقی هدف"><select value={value} onChange={e=>onChange(e.target.value)}><option value="">انتخاب کنید…</option>{living.map(w=><option key={w.id} value={w.trueName}>{w.trueName}</option>)}<option value={UNKNOWN_NAME}>نام ناشناخته یا نادرست</option></select></Field> }
